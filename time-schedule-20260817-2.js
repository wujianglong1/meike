(() => {
  ...
  function updateMergeButton() { const hours = [...selected].sort((a, b) => a - b); mergeButton.disabled = hours.length < 2 || hours.some((hour, index) => index && hour !== hours[index - 1] + 1); }
  ...
  mergeButton.addEventListener('click', () => {
    ...
    if (hours.length < 2 || hours.some((hour, index) => index && hour !== hours[index - 1] + 1)) return;
    const day = S.days[active] || (S.days[active] = fresh());
    day.hourlyMerges = [...ranges(day), { start: hours[0], end: hours.at(-1) }]; selected.clear(); save(); renderSchedule();
  });
