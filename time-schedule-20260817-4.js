(() => {
  ...
  function renderSchedule() {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      choose.addEventListener('change', () => { if (choose.checked) { selected.add(hour); if (selected.size > 2) { const first = Math.min(...selected); selected = new Set([first, hour]); } } else selected.delete(hour); renderSchedule(); });
  ...
  mergeButton.addEventListener('click', () => {
    ...
    const day = S.days[active] || (S.days[active] = fresh());
    const start = Math.min(...hours), end = Math.max(...hours), mergeHours = Array.from({length: end - start + 1}, (_, index) => start + index), source = entriesForDay(day).find(value => value.trim());
    day.hourlyMerges = [...ranges(day), { hours: mergeHours, color: day.hourlyMerges?.length || 0 }];
    if (source) mergeHours.forEach(hour => { day.hourlySchedule[hour] = source; });
