(() => {
  ...
  async function fetchWeather(location, force = false) {
    ...
    if (!force && cached && Date.now() - cached.savedAt < 30 * 60 * 1000 && cached.location.latitude === location.latitude && cached.location.longitude === location.longitude) {
  ...
  function locate() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { status.textContent = '未获得定位权限，请输入城市查询'; },
  ...
  document.querySelector('#weatherRefresh').onclick = () => {
    const location = readJson(locationKey);
    if (location) fetchWeather(location, true); else locate();
  };

  const savedLocation = readJson(locationKey);
  if (savedLocation) {
    if (!savedLocation.name || savedLocation.name === '当前位置') {
      resolveLocationName(savedLocation).then(location => fetchWeather(location, true));
    } else fetchWeather(savedLocation);
  } else locate();
})();
