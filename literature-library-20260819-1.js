(() => {
  ...
  function createPdfReader() {
    ...
    reader.hidden = true;
    reader.innerHTML = '<header class="literature-reader-bar"><button id="literatureReaderBack" class="literature-reader-back" type="button" aria-label="返回文献库" title="返回文献库">←</button><h2 id="literatureReaderTitle"></h2><button id="literatureReaderClose" class="literature-reader-close" type="button" aria-label="关闭阅读器" title="关闭">×</button></header><div class="literature-reader-workspace"><section class="literature-reader-document"><div class="literature-document-toolbar"><span id="literatureDocumentStatus">只读预览</span><div><button id="literatureDocumentEdit" type="button" hidden>编辑正文</button><button id="literatureDocumentSave" type="button" hidden disabled>保存正文</button></div></div><iframe id="literatureReaderFrame" class="literature-reader-frame" title="本地文献阅读器"></iframe><div id="literatureDocumentEditor" class="literature-document-editor" contenteditable="true" role="textbox" aria-multiline="true" data-placeholder="在此编辑文档副本；原始 PDF、Word、PPT 文件保持不变"></div></section><div id="literatureReaderDivider" class="literature-reader-divider" role="separator" aria-label="调整文献与笔记宽度" aria-orientation="vertical" tabindex="0"></div><aside class="literature-notes"><div class="literature-notes-head"><div><span>阅读笔记</span><small id="literatureNoteImportStatus">与当前文献关联保存</small></div><label class="literature-note-import" title="导入 TXT 或 Markdown 笔记文件">导入笔记<input id="literatureNoteFile" type="file" accept=".txt,.md,.markdown,text/plain,text/markdown"></label></div><div class="literature-note-composer"><div class="literature-note-toolbar" role="toolbar" aria-label="正文和笔记文字格式"><button type="button" data-note-command="bold" title="加粗">加粗</button><button type="button" data-note-command="italic" title="斜体">斜体</button><button type="button" data-note-command="underline" title="下划线">下划线</button><button type="button" data-note-command="strikeThrough" title="删除线">删除线</button><button type="button" data-note-command="hiliteColor" data-note-value="#fff1a8" title="高亮">高亮</button><span class="literature-color-label">文字颜色</span><div id="literatureColorPalette" class="literature-color-palette" role="group" aria-label="文字颜色"><button type="button" data-note-command="foreColor" data-note-value="#1f2937" class="color-swatch color-ink" title="深灰色" aria-label="深灰色"></button><button type="button" data-note-command="foreColor" data-note-value="#dc2626" class="color-swatch color-red" title="红色" aria-label="红色"></button><button type="button" data-note-command="foreColor" data-note-value="#d97706" class="color-swatch color-orange" title="橙色" aria-label="橙色"></button><button type="button" data-note-command="foreColor" data-note-value="#16a34a" class="color-swatch color-green" title="绿色" aria-label="绿色"></button><button type="button" data-note-command="foreColor" data-note-value="#2563eb" class="color-swatch color-blue" title="蓝色" aria-label="蓝色"></button><button type="button" data-note-command="foreColor" data-note-value="#7c3aed" class="color-swatch color-purple" title="紫色" aria-label="紫色"></button></div><button type="button" data-note-command="insertUnorderedList" title="项目符号">列表</button><button type="button" data-note-command="formatBlock" data-note-value="blockquote" title="引用">引用</button><button type="button" data-note-command="removeFormat" title="清除格式">清除格式</button></div><small class="literature-formatting-hint">在正文或笔记中选中文字后，直接点击对应颜色</small><div id="literatureNoteInput" class="literature-note-editor" contenteditable="true" role="textbox" aria-multiline="true" data-placeholder="粘贴或写下阅读要点、方法、数据或疑问"></div><div id="literatureNoteResize" class="literature-note-resize" role="separator" aria-orientation="horizontal" tabindex="0" aria-label="调整笔记框高度"></div><button id="literatureNoteSave" type="button">添加笔记</button></div><div id="literatureNotesList" class="literature-notes-list"></div></aside></div>';
    const colorValues = { 'color-ink': '#1f2937', 'color-red': '#dc2626', 'color-orange': '#d97706', 'color-green': '#16a34a', 'color-blue': '#2563eb', 'color-purple': '#7c3aed' };
    reader.querySelectorAll('#literatureColorPalette .color-swatch').forEach(swatch => {
      const color = colorValues[swatch.classList[1]];
      if (color) {
    ...
    const noteResize = $('literatureNoteResize');
    ...
    if (noteResize && noteEditor) {
      const clampHeight = value => Math.max(120, Math.min(520, value));
      const setHeight = value => {
        const next = clampHeight(value);
        noteEditor.style.height = `${next}px`;
        localStorage.setItem('meike-literature-note-editor-height', String(next));
      };
      noteResize.addEventListener('pointerdown', event => {
        noteResize.setPointerCapture(event.pointerId);
        const startY = event.clientY;
        const startHeight = noteEditor.getBoundingClientRect().height;
        reader.classList.add('is-resizing-note');
        const move = moveEvent => setHeight(startHeight + (moveEvent.clientY - startY));
        const finish = finishEvent => {
          try { noteResize.releasePointerCapture(finishEvent.pointerId); } catch {}
          reader.classList.remove('is-resizing-note');
          noteResize.removeEventListener('pointermove', move);
          noteResize.removeEventListener('pointerup', finish);
          noteResize.removeEventListener('pointercancel', finish);
        };
        noteResize.addEventListener('pointermove', move);
        noteResize.addEventListener('pointerup', finish);
        noteResize.addEventListener('pointercancel', finish);
        event.preventDefault();
      });
      noteResize.addEventListener('keydown', event => {
        if (event.key !== 'ArrowUp' && event.key !== 'ArrowDown') return;
        event.preventDefault();
        const step = event.shiftKey ? 40 : 20;
        const current = noteEditor.getBoundingClientRect().height;
        setHeight(current + (event.key === 'ArrowUp' ? -step : step));
      });
    }
    notesPanel.addEventListener('wheel', event => {
      const editor = $('literatureNoteInput');
      const list = $('literatureNotesList');
      const target = event.target instanceof Element && event.target.closest('.literature-note-editor') ? editor : list;
