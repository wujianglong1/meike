(() => {
  ...
  function showEntry(entry){const box=document.createElement('div');box.className='life-entry-viewer';const panel=document.createElement('article');panel.className='life-entry-viewer-panel';const images=normalizedImages(entry),files=normalizedFiles(entry);panel.innerHTML=`<button class="life-entry-viewer-close" type="button" aria-label="关闭">×</button><span class="life-entry-date">${esc(entry.date)}</span><h2>${esc(entry.title||'没有标题的一天')}</h2><p class="life-entry-viewer-note">${esc(entry.note||'')}</p>`;const gallery=document.createElement('div');gallery.className='life-entry-viewer-gallery';images.forEach(image=>{const photo=document.createElement('img');photo.src=urlFor(image.blob);photo.alt=image.caption||'生活照片';photo.onclick=()=>showImage(image.blob);gallery.append(photo)});if(images.length)panel.append(gallery);if(files.length){const attachments=document.createElement('div');attachments.className='life-entry-viewer-files';files.forEach(file=>{const link=document.createElement('a');link.href=urlFor(file.blob);link.download=file.name||'附件';link.target='_blank';link.textContent=`📎 ${file.name||'附件'}`;attachments.append(link)});panel.append(attachments)};const close=()=>box.remove();box.append(panel);box.onclick=event=>{if(event.target===box)close()};panel.querySelector('.life-entry-viewer-close').onclick=close;document.addEventListener('keydown',function escape(event){if(event.key==='Escape'){document.removeEventListener('keydown',escape);close()}});document.body.append(box)}
  function toggleEntry(card,entry){const current=card.querySelector('.life-entry-expanded');if(current){current.remove();card.classList.remove('is-expanded');return}const expanded=document.createElement('div');expanded.className='life-entry-expanded';expanded.innerHTML=`<p class="life-entry-expanded-note">${esc(entry.note||'')}</p>`;const images=normalizedImages(entry),files=normalizedFiles(entry);if(images.length){const gallery=document.createElement('div');gallery.className='life-entry-expanded-gallery';images.forEach(image=>{const photo=document.createElement('img');photo.src=urlFor(image.blob);photo.alt=image.caption||'生活照片';photo.title=image.caption||'';photo.onclick=()=>showImage(image.blob);gallery.append(photo)});expanded.append(gallery)}if(files.length){const attachments=document.createElement('div');attachments.className='life-entry-expanded-files';files.forEach(file=>{const link=document.createElement('a');link.href=urlFor(file.blob);link.download=file.name||'附件';link.target='_blank';link.textContent=`📎 ${file.name||'附件'}`;attachments.append(link)});expanded.append(attachments)}card.append(expanded);card.classList.add('is-expanded')}
  async function render(){
    ...
    entries.forEach(entry=>{const card=document.createElement('article');card.className='card life-entry';const images=normalizedImages(entry),files=normalizedFiles(entry),cover=images[0],tags=entry.tags||[];
      ...
      card.tabIndex=0;card.setAttribute('aria-label','展开生活记录');card.onclick=event=>{if(event.target.closest('button,a,img,.life-entry-expanded'))return;toggleEntry(card,entry)};card.onkeydown=event=>{if((event.key==='Enter'||event.key===' ')&&event.target===card){event.preventDefault();toggleEntry(card,entry)}};card.querySelector('.life-delete').onclick=async()=>{if(confirm('删除这条生活记录？')){await remove(entry.id);render()}};
