(() => {
  ...
  async function render(){
    ...
    entries.forEach(entry=>{const card=document.createElement('article');card.className='card life-entry';const images=normalizedImages(entry),files=normalizedFiles(entry),cover=images[0],tags=entry.tags||[];
      ...
      card.tabIndex=0;card.setAttribute('aria-label','打开生活记录预览');card.onclick=event=>{if(event.target.closest('button,a,img'))return;showEntry(entry)};card.onkeydown=event=>{if((event.key==='Enter'||event.key===' ')&&event.target===card){event.preventDefault();showEntry(entry)}};card.querySelector('.life-delete').onclick=async()=>{if(confirm('删除这条生活记录？')){await remove(entry.id);render()}};
