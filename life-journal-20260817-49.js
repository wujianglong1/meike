(() => {
  ...
  let selectedImages=[],editingId=null,objectUrls=[],searchText='',sortMode='newest';
  ...
  async function updateStorage(entries){
    ...
    const bytes=entries.flatMap(normalizedImages).reduce((sum,image)=>sum+(image.blob.size||0),0),estimate=navigator.storage?.estimate?await navigator.storage.estimate():null;
  ...
  async function exportFullBackup(){
    ...
    try{const records=await all(),life=[];for(const entry of records)life.push({...entry,images:await Promise.all(normalizedImages(entry).map(async image=>({data:await blobToDataUrl(image.blob),caption:image.caption||''})))});
  ...
  async function importFullBackup(file){
    ...
    for(const entry of data.life||[]){const images=await Promise.all((entry.images||[]).map(async image=>typeof image==='string'?{blob:await dataUrlToBlob(image),caption:''}:{blob:await dataUrlToBlob(image.data),caption:image.caption||''}));await put({...entry,images})}
    if(data.daymark)localStorage.setItem('daymark-v1',JSON.stringify(data.daymark));location.reload();
  }
  function resetForm(){editingId=null;selectedImages=[];composer.classList.remove('open');$('#lifeTitle').value='';$('#lifeNote').value='';$('#lifeTags').value='';$('#lifeImages').value='';$('#lifeDate').value=localDate();$('#lifePreview').innerHTML='';$('#saveLifeEntry').textContent='保存记录'}
  ...
  async function render(){
    ...
    if(query)entries=entries.filter(entry=>[entry.title,entry.note,...(entry.tags||[]),...normalizedImages(entry).map(image=>image.caption)].join(' ').toLowerCase().includes(query));
    ...
    entries.forEach(entry=>{const card=document.createElement('article');card.className='card life-entry';const images=normalizedImages(entry),cover=images[0],tags=entry.tags||[];
      card.innerHTML=`${cover?'<img class="life-entry-cover" alt="生活照片">'+(cover.caption?`<small class="life-photo-caption">${esc(cover.caption)}</small>`:''):''}<div class="life-entry-body"><span class="life-entry-date">${esc(entry.date)}</span><h3>${esc(entry.title||'没有标题的一天')}</h3><p>${esc(entry.note)}</p>${tags.length?`<div class="life-tags">${tags.map(tag=>`<span># ${esc(tag)}</span>`).join('')}</div>`:''}</div>${images.length>1?'<div class="life-entry-thumbs"></div>':''}<div class="life-entry-actions"><button class="life-edit" type="button">编辑</button><button class="life-delete" type="button">删除</button></div>`;
      ...
      card.querySelector('.life-edit').onclick=()=>{editingId=entry.id;selectedImages=images.map(image=>({...image}));$('#lifeDate').value=entry.date;$('#lifeTitle').value=entry.title||'';$('#lifeNote').value=entry.note||'';$('#lifeTags').value=tags.join('、');$('#saveLifeEntry').textContent='保存修改';composer.classList.add('open');renderPreview();composer.scrollIntoView({behavior:'smooth',block:'start'})};entriesBox.append(card)});updateStorage(complete);
  }
  $('#newLifeEntry').onclick=()=>{resetForm();composer.classList.add('open');$('#lifeTitle').focus()};$('#cancelLifeEntry').onclick=resetForm;
  $('#lifeImages').onchange=async event=>{const input=event.target,files=[...input.files];input.value='';$('#saveLifeEntry').disabled=true;$('#saveLifeEntry').textContent='正在处理图片…';try{selectedImages.push(...(await Promise.all(files.map(compressImage))).map(blob=>({blob,caption:''})));renderPreview()}finally{$('#saveLifeEntry').disabled=false;$('#saveLifeEntry').textContent=editingId?'保存修改':'保存记录'}};
  $('#saveLifeEntry').onclick=async()=>{const title=$('#lifeTitle').value.trim(),note=$('#lifeNote').value.trim();if(!title&&!note&&!selectedImages.length)return $('#lifeTitle').focus();const tags=$('#lifeTags').value.split(/[，,、]/).map(tag=>tag.trim()).filter(Boolean);await put({id:editingId||`${Date.now()}-${Math.random().toString(16).slice(2)}`,date:$('#lifeDate').value||localDate(),title,note,tags:[...new Set(tags)],images:selectedImages,updatedAt:Date.now()});resetForm();render()};
